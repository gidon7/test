  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/review' && method === 'POST') {
      try {
        ...
        // AI 리뷰
        const aiReview = await getAIReview(codeContent, file.name, env.OPENAI_API_KEY);
...
async function getAIReview(code, filename, apiKey) {
  if (!apiKey) {
    return {
      explanation: 'OpenAI API 키가 설정되지 않았습니다.',
